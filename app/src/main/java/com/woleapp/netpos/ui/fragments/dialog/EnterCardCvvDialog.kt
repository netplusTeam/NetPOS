...
import com.woleapp.netpos.databinding.DialogEnterCvvBinding
import java.util.*

class EnterCardCvvDialog(private val enterCardCvvListener: Listener) : DialogFragment() {
    private var integerList = listOf("1", "2", "3", "4", "5", "6", "7", "8", "9", "0")
    private lateinit var binding: DialogEnterCvvBinding
    ...
    override fun onCreateView(
        ...
        for (i in buttons.indices) {
            buttons[i].setOnClickListener { v: View ->
                if (etPinEt.text.toString().length >= 3) {
        ...
        btnConfirmBtn.setOnClickListener {
            ...
            enterCardCvvListener.onConfirm(etPinEt.text.toString().trim())
            dialog?.cancel()
            dialog?.dismiss()
            dismiss()
        }
